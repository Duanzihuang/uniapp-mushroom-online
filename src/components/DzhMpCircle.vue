<template>
  <div class="mp-circle-container">
    <canvas
      ...
      :canvas-id="'backgroundCanvas'+canvasId"
    ></canvas>
    <canvas
      class="mp-circle"
      :style="{width:width+'px',height:height+'px'}"
      :canvas-id="'foregroundCanvas'+canvasId"
    ></canvas>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    drawProgress() {
      ...
      // 背景色
      const backgroundCtx = wx.createCanvasContext(
        'backgroundCanvas' + this.canvasId
      )
      // 前景色
      const foregroundCtx = wx.createCanvasContext(
        'foregroundCanvas' + this.canvasId
      )
      // 绘制背景色
      ...
      // // 获取文字的宽度
      // const text_width = foregroundCtx.measureText(parseInt(this.progress)+'%').width
      // foregroundCtx.fillText(parseInt(this.progress)+'%',this.width / 2 - text_width / 2,this.height / 2 + font_size / 2)
      if (this.progress >= 99) {
