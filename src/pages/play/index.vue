...
export default Vue.extend({
  ...
  methods: {
    async getCourseDetailData(id) {
      const res = await fetch({ url: `course/play/${id}` })
    ...
    async validatePlayRight() {
      // 检查该用户是否支付了该课程
      const res = await fetch({
        url: `order/paystatus?course_id=${this.course_id}`
      })
      ...
      if (res.data.pay_status === 0) {
        ...
        uni.showModal({
          ...
          success: res => {
            if (res.confirm) {
              uni.navigateTo({
                url: `/pages/pay/index?id=${this.course_id}&title=${this.course_title}&price=${this.course_price}&cover_image_url=${this.cover_image_url}`
              })
            }
          }
        })
